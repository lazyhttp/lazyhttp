...
func NewTabbedPanels() *TabbedPanels {
	t := &TabbedPanels{
		...
		allPages:   make([]*namedPrimitive, 0),
		dividerMid: string(tview.BoxDrawingsDoubleVertical),
		dividerEnd: string(tview.BoxDrawingsLightVertical),
...
func (t *TabbedPanels) SetChangedFunc(handler func()) {
	t.pages.SetChangedFunc(handler)
}
...
func (t *TabbedPanels) AddTab(name, label string, item tview.Primitive) {
	...
	t.pages.AddPage(name, item, true, false)
	t.allPages = append(t.allPages, &namedPrimitive{name, item})
...
func (t *TabbedPanels) RemoveTab(name string) {
	t.pages.RemovePage(name)
	// remove from allPages where Name is name
	t.allPages = slices.DeleteFunc(t.allPages, func(p *namedPrimitive) bool {
		return p.Name == name
	})
...
func (t *TabbedPanels) updateVisibleTabs() {
	...
	for _, panel := range allPanels {
		if panel.Name == t.currentTab {
			t.pages.ShowPage(panel.Name)
		} else {
			t.pages.HidePage(panel.Name)
		}
	}
}
